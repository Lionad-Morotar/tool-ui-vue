...
- fix(components): data-table 列宽不再被表格 w-full 压缩——min-width 下限钳制总宽，resizer 拖拽真实生效；手柄列头 hover 显现并加宽热区，补发 columnResize 事件；总宽超出容器后横向可滚动

## 0.3.17-alpha.2

### Patch Changes

- feat(components): QuestionFlow 步骤支持表单字段——fields 复用 PreferenceItem 契约，options/fields 二选一校验，required 填齐门控推进，complete 载荷按步骤类型分化
