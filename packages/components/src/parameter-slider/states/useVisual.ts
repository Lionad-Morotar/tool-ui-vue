...
export interface UseVisualOptions {
  sliders: ComputedRef<SliderConfig[]>;
...
export function useVisual(options: UseVisualOptions): VisualReturns {
  const { sliders, getSliderValue, getSliderRowState } = options;

  // Helper to get slider config by id
  function getSliderConfig(sliderId: string): SliderConfig | undefined {
    return sliders.value.find((s) => s.id === sliderId);
  }
