...
import type { OptionListProps } from './schema';

defineOptions({ name: 'CmptOptionList', inheritAttrs: false })

const props = withDefaults(defineProps<OptionListProps & { modelValue?: import('./schema').OptionListSelection } & { css?: { root?: string } }>(), {
  css: () => ({ root: '' })
})

const emit = defineEmits<{
  change: [value: import('./schema').OptionListSelection];
  action: [actionId: string, value: import('./schema').OptionListSelection];
  'update:modelValue': [value: import('./schema').OptionListSelection];
}>()
